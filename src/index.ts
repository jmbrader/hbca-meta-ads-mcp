...
const ALLOWED_DESTINATION_HOSTS = (
  process.env.ALLOWED_DESTINATION_HOSTS || ""
)
  .split(",")
...
function validateDestinationUrl(value: string): string {
  ...
  if (
    ...
    !ALLOWED_DESTINATION_HOSTS.includes(
...
function createServer() {
  const server = new McpServer(
    ...
    {
      instructions:
        "Read and controlled-write access to the configured HBCA Meta Ads account. Never claim a change succeeded unless the relevant write tool returned a successful Meta response. New ads must be created PAUSED. Creative replacement is allowed only on paused ads. Writes are restricted to entities belonging to the configured ad account. Budget increases are limited by server-side safety rules.",
    }
  );
  ...
  server.registerTool(
    ...
    async (args: any) => {
      ...
      const data = await metaGet(
        `${DEFAULT_AD_ACCOUNT_ID}/campaigns`,
        {
          fields:
  ...
  server.registerTool(
    ...
    async (args: any) => {
      ...
      const data = await metaGet(
        `${DEFAULT_AD_ACCOUNT_ID}/insights`,
        params
      );
  ...
  server.registerTool(
    "get_ad_images",
    {
      ...
      description:
        "List images already stored in the configured Meta ad account, including image hashes, names, dimensions, and preview URLs.",
      inputSchema:
        z.object({
          limit:
            z.number()
    ...
    async (args: any) => {
      ...
      const data = await metaGet(
        `${DEFAULT_AD_ACCOUNT_ID}/adimages`,
        {
          fields:
            "id,name,hash,url,width,height,created_time",
          limit,
        }
      );
  ...
  server.registerTool(
    "get_ad_creative",
    {
      title: "Get Meta ad creative",
      description:
        "Inspect one Meta ad creative, including its Page/story configuration, destination link data, image hash, headline, body text, and CTA when available.",
      inputSchema:
        z.object({
          creative_id:
            z.string().min(1),
    ...
    async (args: any) => {
      ...
      const data = await metaGet(
        ...
        {
          fields:
            "id,name,account_id,object_story_id,object_story_spec,asset_feed_spec,body,title,call_to_action_type,image_hash,image_url,link_url,thumbnail_url,url_tags,status",
        }
      );
  ...
  const statusTool = (
    ...
    server.registerTool(
      name,
      {
        title,
        description:
          `${title} after verifying account ownership.`,
        inputSchema:
          z.object({
            [idField]:
              z.string().min(1),
  ...
  server.registerTool(
    ...
    {
      title:
        "Replace creative on a paused Meta ad",
      description:
  ...
  (
    ...
    res.json({
      ...
      service:
        "hbca-meta-ads-mcp",
      mode:
        "controlled-read-write-creative",
      version:
        "3.1.0",
      guardrails: {
        configured_account:
        ...
        max_daily_budget_usd:
          MAX_DAILY_BUDGET_USD,
        new_ads_created_paused:
          true,
        creative_replacement_requires_paused_ad:
          true,
        max_image_bytes:
          MAX_IMAGE_BYTES,
        destination_host_allowlist_enabled:
          ALLOWED_DESTINATION_HOSTS.length >
          0,
      },
    });
  }
